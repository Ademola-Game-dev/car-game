...
namespace World.Culling {
    public class WorldChunksStreamer : MonoBehaviour {
        public GameDetector.Game CurrentGame = GameDetector.Game.MostWanted;
        ...
        private Dictionary<string, LoadBigChunkTask> CurrentlyAsyncLoadingChunks = new Dictionary<string, LoadBigChunkTask>();

        private Dictionary<string, List<GameObject>> matLinks = new Dictionary<string, List<GameObject>>();

        private List<CoordDebug.StreamingSectionInfo> streamSectionsInfo;
        private List<CoordDebug.StreamingSection> streamSections;
        ...
        public string dataFileMostWanted = "E:/Games/NFS/Need For Speed Most Wanted REDUX 3.04 - Main/TRACKS - Copy/L2RA.BUN";
        public string modelsFileMostWanted = "E:/Games/NFS/Need For Speed Most Wanted REDUX 3.04 - Main/TRACKS - Copy/STREAML2RA.BUN";

        private void Start() {
            Initialize();
            RequestInitialNeededChunks();
            StartCoroutine(ChunksAutoOnDemandLoaderCoroutine());
        }
        ...
        public void Initialize() {
            ...
            string modelsFileToUse;

            switch (CurrentGame) {
                case GameDetector.Game.MostWanted:
                    dataFileToUse = dataFileMostWanted;
                    modelsFileToUse = modelsFileMostWanted;
                    break;
                default:
                    Debug.LogError("Unsupported game selected");
                    return;
            }

            FileStream f2 = new FileStream(modelsFileToUse, FileMode.Open, FileAccess.Read, FileShare.Read);
            ...
            byte[] fSmol = File.ReadAllBytes(dataFileToUse);
            ...
            // Populăm chunkData cu datele despre secțiuni
            for (int i = 0; i < streamSections.Count; i++) {
                var section = streamSections[i];
                var info = streamSectionsInfo[i];
                Chunk chunk = new Chunk(section.SectionName);
                chunk.section = section;
                chunk.sectionInfo = info;

                CoordDebug.LoadSectionData(f, section, out chunk.sectionInfo.instanceArray, out chunk.sectionInfo.infoArray, out chunk.sectionInfo.actualIndex);

                chunkData.Add(chunk.ID, chunk);
            }
            ...
            while (true) {
                for (int i = 0; i < data.Length; i++) {
                    ...
                    if (!LoadedChunksNames.Contains(cur.ID)) {
                        if (cur.section.Radius == 0f || Vector2.Distance(new Vector2(mainCam.position.x, mainCam.position.z), centres[i]) <= cur.section.Radius + drawDistance) {
                            InitiateBigChunkLoading(cur);
                            yield return 0;
                        }
                    } else if (LoadedChunks.ContainsKey(cur.ID)) {
                        if (Vector2.Distance(new Vector2(mainCam.position.x, mainCam.position.z), centres[i]) > cur.section.Radius + drawDistance) {
