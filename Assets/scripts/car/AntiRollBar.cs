...
public class AntiRollBar : MonoBehaviour {
	...
	void FixedUpdate ()
	{
		...
		if (groundedL)
			carStateMachine.rigidbody.AddForceAtPosition (WheelL.transform.up * -antiRollForce, WheelL.transform.position);

		if (groundedR)
			carStateMachine.rigidbody.AddForceAtPosition (WheelR.transform.up * antiRollForce, WheelR.transform.position);
	}
}
